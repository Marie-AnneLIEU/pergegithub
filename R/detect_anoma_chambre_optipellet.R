...
#' @return \code{list.data} liste de jeux de données après traitements
#' @return \code{tab_res} tableau de conclusions
#' @return \code{tab_ad} tableau de conclusions avec colonnes non renommées
#' @return \code{missing_column} liste de jeux de données avec les colonnes manquantes
#' @return \code{na_col_names} liste de jeux de données dont le nombre de lignes = 0 après la suppression des NAs
#' @return \code{deleted_room_names} liste de jeux de données dont la chambre/zone à chauffer numéro \code{num_chambre} n'est pas déclarée
#' @return \code{deleted_period_names} liste de jeux de données dont aucune période de fonctionnement est détectée
#' @export
...
detect_anoma_chambre_optipellet <- function(list.data,num_chambre,seuil_data = 12,k_pas = 6,s = 0.0065,seuil_delete=5,seuil_cut=21
                                 ,seuil_stat=5e-5,t_eau_maxi=90){

  require(lubridate)

  ## detecter les colonnes manquantes----
  list_names <- names(list.data)
  missing_column <- missing_column_check_optipellet(list.data,num_chambre)
  ...
  if(length(list.data)>0){
    ...
    res_na <- clean_na_all_optipellet(list.data,num_chambre)
        ...
        if(length(list_period)>0){
          ...
          if(length(list_period)>0){
            ...
            for (i in 1:length(list.data)){
              ...
              tab_cons<-data.frame(date,indice_s,indice_e,heure_start[-1],heure_end[-1],fin,t_cons,ecs,table,signe,loi_att,t_max_a)
              ...
              cond1 <- si[si %notin% f]
              conclusion[cond1] <- "Pièce(s) défectueuse(s)/puissance insuffisante"
              ...
              tab_add <- data.frame(tab_cons, conclusion,pertinence)
