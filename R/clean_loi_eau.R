...
#' @param P_DTCHAUD delta chaudiére
...
#' @inherit C_DAMBcal
#' @export
clean_loi_eau <- function(list.data,P_PENTE,P_DECPA,P_TEXTMAX,P_DTCHAUD,P_TMAX,seuil_ecart,P_SEUIL,P_KCOMP){
  ...
  ind_del <- which(ecart > seuil_ecart)
  if(length(ind_del)>0){
